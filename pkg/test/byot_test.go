...
//
// On real hardware the upgrade reaches Upgraded (no error). In Talos-in-Docker
// the LifecycleClient.Upgrade RPC is unsupported, so the upgrade settles
// Failed with reason UpgradeFailed and a "container mode" message — that
// specific failure is tolerated (the state machine still ran end-to-end: wiring
// → probe → Upgrade issued → Failed). Any other terminal failure reason
// (VersionProbeFailed, InvalidImageRef) is a genuine wiring/auth bug and fails
// the test with the condition reason+message, rather than an opaque timeout.
func assertByotMachinesUpgradeSettled(
	...
	for _, name := range names {
		status, err := helpers.WaitForByotMachineUpgradeSettled(
		...
		if err == nil {
			t.Logf("ByotMachine %s upgrade settled Upgraded: current=%q applied=%q",
				name, status.CurrentTalosVersion, status.UpgradeAppliedImageRef)

			continue
		}

		var failed *helpers.ByotUpgradeFailedError
		require.ErrorAs(t, err, &failed,
			"ByotMachine %s upgrade settled with a non-failure error: %v", name, err)
		require.Equal(t, helpers.ByotUpgradeReasonUpgradeFailed, failed.Reason,
			"ByotMachine %s upgrade failed with unexpected reason %q (message: %s)",
			name, failed.Reason, failed.Message)
		require.Contains(t, failed.Message, "container mode",
			"ByotMachine %s upgrade failed for a non-container-mode reason %q (expected the Talos-in-Docker "+
				"\"method is not supported in container mode\" limitation): %s",
			name, failed.Reason, failed.Message)

		t.Logf("ByotMachine %s upgrade settled Failed (container mode): %s", name, failed.Message)
	}
}
