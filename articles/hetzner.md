...
`helm template | kubectl apply`, as above, is also the upgrade path. A plain
`helm upgrade` conflicts on `.spec.controlPlaneEndpoint.host`: the chart
templates it empty because CAPH needs the endpoint to exist before it creates
the load balancer, and CAPH then fills in the load balancer address and owns the
field. `helm upgrade --force` resolves it, at the cost of forcing every field.

`hcloudNetwork`, `controlPlaneLoadBalancer.enabled`, `controlPlaneEndpoint.port`
and the load balancer region are immutable in CAPH. Kommodity does not run
CAPH's validating webhook, so changing any of them is accepted by the API server
and then ignored by the controller. Recreate the cluster instead.
...
`make run-hetzner-integration-test` provisions a real cluster, waits for the
workload API, and tears it down. It needs three things that the target does not
create:

- `HCLOUD_TOKEN` in the environment.
- A current `kommodity:latest` image, from `make build-image`. The test runs
  Kommodity in a container, so a stale image fails with `infrastructure provider
  is not supported` before it reaches Hetzner at all.
- A snapshot named by `talos.imageName` in
  [`values.hetzner.yaml`](../charts/kommodity-cluster/values.hetzner.yaml),
  built and uploaded as above.
