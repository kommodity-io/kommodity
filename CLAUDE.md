...
- Tests must use `github.com/stretchr/testify`'s `assert`/`require` packages instead of manual `if err != nil { t.Fatalf(...) }`/`t.Errorf(...)` checks. Use `require` for checks that should stop the test immediately (for example `require.NoError`, `require.NotNil`) and `assert` for checks that should report a failure but let the test keep running (for example `assert.Equal`, `assert.ErrorIs`).
