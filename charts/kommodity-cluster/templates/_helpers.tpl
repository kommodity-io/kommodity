...
{{/*
kommodity-cluster.byotValidateReplicas — validate the replica count for a
byot pool. Control plane replicas must be odd and >= 1 (etcd quorum) and are
always required (TalosControlPlane.replicas; no autoscaling for CP). Worker
nodepool replicas are required by default, but MAY be omitted when
autoscaling.{minReplicas,maxReplicas} is set (the MachineDeployment is then
sized by minReplicas; byot claims hosts as replicas grow up to maxReplicas).
Input: dict "poolValues" (controlplane/nodepool values) "scope" (value path)
"controlPlane" (bool: true for CP, false for workers).
*/}}
{{- define "kommodity-cluster.byotValidateReplicas" -}}
{{- $p := .poolValues -}}
{{- $hasReplicas := not (kindIs "invalid" (dig "replicas" nil $p)) -}}
{{- $minReplicas := dig "autoscaling" "minReplicas" nil $p -}}
{{- $maxReplicas := dig "autoscaling" "maxReplicas" nil $p -}}
{{- $hasMin := not (kindIs "invalid" $minReplicas) -}}
{{- $hasMax := not (kindIs "invalid" $maxReplicas) -}}
{{- $hasAutoscaling := and $hasMin $hasMax -}}
{{- if .controlPlane -}}
{{- if not $hasReplicas -}}
{{- fail (printf "%s.replicas is required for Byot control plane (odd, >= 1, for etcd quorum)" .scope) -}}
{{- end -}}
{{- $r := int $p.replicas -}}
{{- if lt $r 1 -}}
{{- fail (printf "%s.replicas must be >= 1 for Byot, got %v" .scope $p.replicas) -}}
{{- end -}}
{{- if eq (mod $r 2) 0 -}}
{{- fail (printf "%s.replicas must be odd for the Byot control plane (etcd quorum), got %v" .scope $p.replicas) -}}
{{- end -}}
{{- else -}}
{{- /* Workers: replicas required unless autoscaling is set. */ -}}
{{- if and (not $hasReplicas) (not $hasAutoscaling) -}}
{{- fail (printf "%s.replicas is required for Byot (no default; ensure enough Available ByotHosts match the pool selector)" .scope) -}}
{{- end -}}
{{- if $hasReplicas -}}
{{- $r := int $p.replicas -}}
{{- if lt $r 1 -}}
