...
| [azurerm_public_ip.egress](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/public_ip) | resource |
...
| [azurerm_subnet_nat_gateway_association.container](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/subnet_nat_gateway_association) | resource |
...
| <a name="input_ingress_ip_restrictions"></a> [ingress\_ip\_restrictions](#input\_ingress\_ip\_restrictions) | IP/CIDR restrictions on the Container App ingress. Empty list = all traffic allowed. Add Allow entries to make a whitelist; unmatched traffic is denied. | <pre>list(object({<br/>    cidr        = string<br/>    action      = optional(string, "Allow")<br/>    name        = string<br/>    description = optional(string, "")<br/>  }))</pre> | `[]` | no |
...
| <a name="input_nat_gateway"></a> [nat\_gateway](#input\_nat\_gateway) | NAT gateway for stable Container App outbound IP. Attach to container subnet. | <pre>object({<br/>    enabled      = optional(bool, false)<br/>    idle_timeout = optional(number, 30)<br/>    # Set to null for non-zonal regions; "1"-"3" where availability zones exist.<br/>    zone = optional(string, null)<br/>  })</pre> | `{}` | no |
...
| <a name="output_container_app_egress_ip"></a> [container\_app\_egress\_ip](#output\_container\_app\_egress\_ip) | The static outbound IP of the Kommodity Container App when NAT gateway is enabled; null otherwise. |
